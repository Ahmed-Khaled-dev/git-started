...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            { 
                if (!dialogue_text.script_ended && current_screen == "levels" && continuation_message.sub_script_ended)
                {
...
void showContinuationMessage(continuationMessage& continuation_message)
{
    ...
    if(!dialogue_text.script_ended && continuation_message.sub_script_ended)
    {
        ...
        continuation_message.continuation_text.setPosition(1047, 950);
    }
